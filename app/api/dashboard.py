...
async def settings_profile(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    Профиль (имя / email / телефон) теперь читается read-only
    из последнего резюме (``resumes.parsed_data``). Редактируются
    эти поля в hh.ru, а не у нас — эндпоинт просто отвечает ``ok``,
    чтобы старая кнопка «Сохранить изменения» не падала.
    """
...
async def settings_preferences(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    Желаемая позиция / стек / зарплата теперь приходят из
    резюме (``resumes.parsed_data``) и не редактируются вручную.
    """
    await _require_user(request, db)
    return {"ok": True}
