...
class PrefsView:
    ...
    # ── Поля, дочитываемые из ``resumes.parsed_data`` (override) ────
    # ``grade`` / ``stop_words`` / ``stop_companies`` редактируются в
    # ``/settings`` и попадают сюда же через ``_save_resume_overrides``.
    @property
    def grade(self) -> str | None:
        v = self._parsed.get("grade")
        return str(v) if v else None

    @property
    def stop_words(self) -> str | None:
        v = self._parsed.get("stop_words")
        return str(v) if v else None

    @property
    def stop_companies(self) -> str | None:
        v = self._parsed.get("stop_companies")
        return str(v) if v else None
