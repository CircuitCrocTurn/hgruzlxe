...
def classify_chat_status(
    *,
    ...
    Логика:
      1. **В ожидании (жёлтый)** — последнее сообщение от нас
         (``last_message_outgoing=True``): ждём ответа работодателя.
      2. **Отказ (красный)** — работодатель ответил И либо
         ``applicant_state`` в группе DISCARD, либо текст последнего
         сообщения совпал с шаблоном отказа (:func:`is_refusal_text`).
      3. **Заинтересованы (зелёный)** — работодатель ответил И не отказ.
         Сюда же попадает кейс «после Отказа работодатель что-то написал»
         (он передумал): новое входящее сообщение перетирает
         ``applicant_state`` в БД на тот, что у него прикреплён (обычно
         None для обычного текста), и кейс попадает в эту ветку.
    """
...
def _parse_dt(s: str | None) -> datetime | None:
    ...
    Обычно hh.ru присылает ISO с offset'ом (``+03:00``), и
    ``datetime.fromisoformat`` его подхватывает. Фоллбэк через
    ``split(".")[0]`` теряет tz — в этом случае считаем строку
    UTC, чтобы получить детерминированное сравнение.
    """
    if not s:
        return None
    try:
