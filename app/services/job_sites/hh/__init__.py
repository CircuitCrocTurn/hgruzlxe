...
async def _stdin_code_provider(prompt: str) -> str:
    """Дефолтный провайдер: спрашиваем у пользователя в консоли."""
...
class HHClient(BaseJobSiteClient):
    ...
    async def ensure_logged_in(self) -> None:
        """Главный entrypoint: использует кэш или проходит полный логин."""
        if await self.is_authenticated():
            ...
            return
        logger.info("сессия отсутствует или истекла — полный логин")
        await self.login()

    async def login(self, code_provider: Optional[CodeProvider] = None) -> None:
        """Полный OTP-флоу: SMS → ввод кода → авторизация."""
        provider = code_provider or _stdin_code_provider

        # Чистим протухшие auth-куки, иначе DataDome может зависнуть на
        # challenge-странице, увидев несвежие __ddg*.
        self._reset_stale_cookies()

        logger.info("warm_up: GET %s", BASE)
        await self._warm_up()
        logger.info("warm_up: ok, _xsrf=%s…", (self._xsrf or "")[:8])

        logger.info("отправляю SMS на %s…", self.phone)
        otp = await self._send_sms()

        formatted = otp["otp"]["formattedPhone"]
        length = otp["codeLength"]
        next_send = otp["otp"]["secondsUntilNextSend"]
        logger.info("✓ SMS отправлен на %s, %s цифр, повтор через %sс",
                    formatted, length, next_send)

        code = await provider(f"Код из SMS на {formatted} ({length} цифр): ")
        logger.info("отправляю код…")
        await self._submit_code(code)

        # После логина _xsrf обновился — синхронизируем
        self._xsrf = self._cookie("_xsrf")
        await self._persist_cookies()
        logger.info("✓ авторизованы, hhuid=%s", self._cookie("hhuid"))
    ...
    * ``responses``               — список ответов hh.ru на каждый
    ...
    async with HHClient.from_context(ctx) as client:
        ...
        if resume_txt is None:
            ...
            resume_txt = (await client.get_resume(resume_id)).model_dump()
