...
# Хабр шлёт ``createdAt`` в виде ``"YYYY-MM-DD HH:MM:SS"`` (по их
# наблюдаемому времени — MSK). Без явного offset'а ``fromisoformat``
# вернёт naive datetime, а у нас в БД tz-aware. Делаем явный fallback:
# трактуем такую строку как Europe/Moscow и переводим в UTC.
...
def _parse_habr_dt(s: str | None) -> datetime | None:
    """``"2026-05-17 12:47:40"`` (MSK) или ISO → tz-aware UTC."""
    if not s:
        return None
    s = s.strip()
    if not s:
    ...
    if dt.tzinfo is None:
        if _MSK_TZ is not None:
            ...
            # Платформа без tzdata — считаем UTC, чтобы не падать.
...
def _build_chat_row(item: dict, *, current_user_alias: str | None) -> _ChatRow:
    ...
    last_msg_body = last_message.get("body") or ""
