    ...
    * :func:`app.queue.context.load_worker_context` — если на момент
    ...
    if True:
        async with HHClient(
            ...
            # ``ensure_logged_in`` сам пометит ``platform_credentials``
            # юзера как ``expired`` и кинет ``HHAuthError``, если сессия
            # невалидна. Интерактивного re-login из background-кода нет
            # — юзер сам переподключит площадку из ``/settings``.
...
async def work_at_all_one_platform(ctx: WorkerContext, slug: str):
    ...
    if cls is None:
        work_logger.warning(
            "user=%s platform=%s: no client class registered",
            ctx.user_id, slug,
        )
        return
    async with cls.open_for(ctx) as client:
        try:
            ...
            await client.respond_to_vacancies(ctx)
            work_logger.info(
                "user=%s platform=%s: would use %s",
                ctx.user_id, slug, cls.__qualname__,
            )
        except NotImplementedError:
            # Клиент площадки ещё не умеет ``respond_to_vacancies`` —
            # это нормально для платформ-stub'ов, пропускаем.
            pass
        except HHAuthError:
            # Сессия протухла. ``ensure_logged_in`` внутри клиента уже
            # пометил ``platform_credentials.status='expired'`` —
            # просто завершаем проход тихо, не валим воркер. Юзер
            # увидит бейдж в ``/settings`` и переподключится.
            work_logger.warning(
                "work_at_all: hh-сессия expired, проход прерван "
                "(user=%s)", ctx.user_id,
            )
