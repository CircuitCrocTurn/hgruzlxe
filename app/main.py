...
from app.services.realtime import start_hub, stop_hub
...
async def lifespan(app: FastAPI):
    ...
    # Live-обновления чатов через Postgres LISTEN/NOTIFY. Хаб держит
    # выделенный asyncpg-коннект; падение/рестарт PG ловится внутри
    # (см. ``ChatHub._listener_loop``), наружу ошибок не отдаёт.
    try:
        await start_hub()
    except Exception:
        logger.exception("start_hub on startup failed")

    yield

    # Сначала останавливаем live-хаб (закрывает LISTEN-коннект и будит
    # всех WebSocket-подписчиков), потом шедулер, потом engine.
    try:
        await stop_hub()
    except Exception:
        logger.exception("stop_hub on shutdown failed")
